...
void initializeSignalGo() {
  try {
    ...
    PostJsonToServerService.baseUrl = "http://${host}:${port}/";
    HelloworldServiceDuplex.current = new HelloworldServiceDuplex();

    provider.autoReconnectAsync(host, port, connectionChanged);
  } catch (ex) {
    print(ex);
  }
}

void connectionChanged(bool value) async {
  try {
    if (value) {
      while (true) {
        var result = await HelloworldServiceDuplex.current.getUserInfoes();
        print(result);
      }
    }
  } catch (ex) {
